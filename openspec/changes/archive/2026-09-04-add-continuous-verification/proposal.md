...
- Add a Rust lane on push to `main` and on pull requests: format, lint, and the workspace test suite, on Linux, macOS, and Windows.
...
- `docs/docs/development/testing.md`, `README.md` — both describe running the suite by hand as the only way it is run, and both quote a test count that nothing verified. They said 228 and 190 respectively; the workspace had 401 when this change was implemented and has 409 after the exec-form regression coverage.
