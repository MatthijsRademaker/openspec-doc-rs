...
**It overlaps `add-comment-thread-actions`**, which is unstarted and extends the same capability with an `addressed` status. Both touch `Status`, `Event`, and the same CLI subcommand group. They should either land in a deliberate order or be merged; landing them in parallel means two changes editing the same enum. The exploration recommends re-scoping `add-comment-thread-actions` to its core and CLI half, at which point merging the two is the smaller option.
