...
Deliberately **not** in scope: no state file, for the reasons `add-dashboard-lifecycle`'s design sets out at length — a gitignored record of port and pid is deleted by `git clean -xdf` while its server keeps running, manufacturing the exact fleet of undiscoverable orphans this command exists to prevent. No `serve restart`, which is `kill` followed by the hooks doing what they already do. No management of dashboards on remote hosts.
