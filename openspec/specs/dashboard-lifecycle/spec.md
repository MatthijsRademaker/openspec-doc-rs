...
Discovery compares canonical roots, so the comparison is exact rather than heuristic. The process id is reported because it is what remains when the shutdown route cannot be reached: a dashboard is normally stopped over the port it serves, and the pid is the manual escape hatch for one so wedged it no longer answers HTTP.
