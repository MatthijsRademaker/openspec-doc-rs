# agent-hook-bridge Specification

## Purpose

The turn-end bridge between a coding agent and this project's review directives: reading a Claude Code or pi.dev turn-end payload from stdin, normalizing it into an agent-agnostic event, looking up and consuming the session's pending directive, and emitting that agent's block-and-continue or allow-stop decision. Claude Code is driven by its native Stop hook; pi.dev has no external-process hook, so its half of the bridge is a project-local TypeScript extension that speaks the wire format defined here.
