...
The system SHALL compose each session and change route from the shared observatory visual system as an instrument header, a primary document spine, spatially anchored conversation, and persistent decision controls. Celestial artwork SHALL frame those regions without obscuring document text, state, focus, or controls.

#### Scenario: Desktop keeps document primary

- **WHEN** a scope page is rendered at 1280 CSS pixels or wider
- **THEN** the artifact SHALL retain a readable primary document spine while operational metadata and anchored conversation occupy distinct instrument regions

#### Scenario: Anchored conversation enters narrow document flow

- **WHEN** a scope page is rendered at 390 CSS pixels wide
- **THEN** metadata SHALL precede the artifact, each expanded anchored thread SHALL follow its block in document flow, persistent controls SHALL remain reachable, and the page SHALL have no horizontal overflow

#### Scenario: Artwork yields before review content

- **WHEN** available width cannot preserve both celestial framing and review content
- **THEN** the artwork SHALL shrink, crop, or disappear before any artifact text, identifier, state, thread, or control is hidden or clipped

#### Scenario: Scope chrome reports only real state

- **WHEN** a scope page renders instrument chrome inspired by the visual source
- **THEN** it SHALL NOT present repository metadata, activity events, navigation destinations, or review state absent from the scope or index APIs
