...
- Put reusable state colors in central semantic tokens (`open`, `addressed`, `resolved`,
  `verdict`, `delivery`, `reviewer`, `agent`) in both `:root` and `.dark`. Do not paste raw
  status colors into a component.
...
Consult `designs/dashboard-review-workbench.md` before styling. Preserve document-first
review hierarchy, visible Reviewer/Agent labels, keyboard/focus behavior, narrow layouts,
offline assets, and restrained motion. A generic stock component demo is not completion.
