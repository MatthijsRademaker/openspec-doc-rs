...
| Release is source-only; setup is lossy | Moderate | **Versioned source release automation is in place.** `release-plz` opens a reviewed release PR, then creates a tag, changelog, and GitHub release; no binaries are included yet — `add-release-binaries-and-installers` owns that remaining half. Hook config lives in gitignored local settings, so a fresh clone has none and nothing says so — the hooks in *this* repo were absent for an unknown period without anyone noticing. `add-setup-diagnostics` covers that setup half. `.github/workflows/rust.yml` runs `make check` and `make test` on Linux and macOS for every push and pull request. |
